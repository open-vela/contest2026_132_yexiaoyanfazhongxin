...
function reporters(runner) {
  ...
  runner.on(constants.EVENT_FAIL, function(title, test, err) {
    if (arguments.length === 1) {
      failures++;
      const fmt = indent(1) + color('fail', `  × ${title}`);
      console.log(fmt);
    } else {
      const caseInfo = casesInfo.get(test)
      if (caseInfo) {
...
function bdd(context, root) {
  ...
  context.it = context.specify = function (title, fn) {
    ...
    // 白名单过滤：如果设置了 __onlyCases__，只跑白名单里的 case
    if (typeof global.__onlyCases__ !== 'undefined' && global.__onlyCases__.length > 0
        && !global.__onlyCases__.includes(title)) {
      test.isPending = true;
    }
